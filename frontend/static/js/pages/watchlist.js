...
export async function mountWatchlist(root, ctx) {
  ...
  // Earnings + sector sidebar clicks
  root.querySelector('#watch-sidebar').addEventListener('click', e => { const b=e.target.closest('[data-ticker]'); if(b) { e.preventDefault(); openModal(b.dataset.ticker); } });
}

function sidebar(status, earnings, sectors) {
  const market = status.market || 'unknown';
  const phase = status.phase || '';
  const phaseMap = { 'pre-market': '盘前交易', 'regular': '交易中', 'after-hours': '盘后交易', 'weekend': '周末休市', 'overnight': '休市', 'open': '交易中', 'closed': '已休市' };
  const phaseLabel = phaseMap[phase] || phaseMap[market] || market;
  const isOpen = /open|regular/i.test(market);
  const es = (earnings.earnings || []).slice(0,5);
  const ss = (sectors.sectors || []).slice(0,4);
  return `<div class="bg-surface-container-lowest p-6 rounded-2xl space-y-4"><h3 class="text-xs font-black font-headline tracking-widest uppercase text-on-surface-variant flex items-center gap-2"><span class="material-symbols-outlined text-primary text-lg">schedule</span>市场状态</h3><div class="flex items-center justify-between"><span class="text-2xl font-black font-headline">${phaseLabel}</span><span class="w-3 h-3 rounded-full ${isOpen?'bg-tertiary animate-pulse':'bg-error'}"></span></div><p class="text-xs text-on-surface-variant">${status.server_time ? new Date(status.server_time).toLocaleString('zh-CN') : ''}</p></div>
  <div class="bg-surface-container-lowest p-5 rounded-2xl space-y-4"><h3 class="text-xs font-black font-headline tracking-widest uppercase text-on-surface-variant">即将公布财报</h3><div class="space-y-3">${es.map(e=>`<button data-ticker="${e.ticker}" class="w-full flex items-center justify-between p-3 bg-surface-container-low rounded-xl text-left hover:bg-surface-container hover:-translate-y-0.5 hover:shadow-md transition-all cursor-pointer"><div><p class="font-bold text-sm text-primary">${e.ticker}</p><p class="text-[10px] text-on-surface-variant line-clamp-1">${e.name || ''}</p></div><div class="text-right shrink-0"><p class="text-xs font-bold">${e.earnings_date || ''}</p><p class="text-[10px] text-on-surface-variant">EPS ${e.eps_estimate != null ? Number(e.eps_estimate).toFixed(2) : '—'}</p></div></button>`).join('') || '<p class="text-sm text-on-surface-variant">暂无数据</p>'}</div></div>
  <div class="bg-surface-container-lowest p-5 rounded-2xl space-y-4"><h3 class="text-xs font-black font-headline tracking-widest uppercase text-on-surface-variant">板块 IV 概览</h3>${ss.map(s=>`<a href="#sectors" class="flex items-center gap-3 p-3 bg-surface-container-low rounded-xl hover:bg-surface-container transition"><span class="material-symbols-outlined text-primary">bubble_chart</span><div><p class="font-bold text-sm">${s.name}</p><p class="text-[10px] text-on-surface-variant">${(s.tickers||[]).length} 只标的</p></div></a>`).join('')}</div>`;
}
