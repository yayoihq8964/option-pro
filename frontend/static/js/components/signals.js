...
export function renderSignals(data) {
  if (!data || data.__error) {
    return `<div class="rounded-[2rem] p-6 md:p-8 text-white bg-gradient-to-br from-[#6a1cf6] to-[#4953ac] shadow-2xl relative overflow-hidden min-h-[320px] flex items-center justify-center text-white/70 font-medium">Technical Signals 暂不可用</div>`;
  }
  const s = data.signals || {};
  const rows = [
    ['RSI(14)', s.rsi?.value ?? s.rsi, s.rsi?.status ?? s.rsi?.signal],
    ['MACD', s.macd?.value ?? s.macd, s.macd?.status ?? s.macd?.signal],
    ['EMA(20)', s.ema20?.value ?? s.ema20, s.ema20?.status ?? s.ema20?.signal],
    ['SMA(50)', s.sma50?.value ?? s.sma50, s.sma50?.status ?? s.sma50?.signal],
    ['Volume', s.volume?.value ?? s.volume, s.volume?.status ?? s.volume?.signal],
  ];
  ...
  return `<div class="rounded-[2rem] p-6 md:p-8 flex flex-col text-white bg-gradient-to-br from-[#6a1cf6] to-[#4953ac] shadow-2xl shadow-primary/20 relative overflow-hidden min-h-[320px]">
    ...
    <div class="relative z-10 flex flex-col h-full">
      <div class="flex items-center gap-3 mb-6">
        ...
        <h3 class="font-headline font-extrabold text-lg tracking-tight">Technical Signals</h3>
      </div>
      <div class="flex justify-between items-end mb-4"><span class="text-[10px] font-bold text-white/70 uppercase tracking-[0.15em]">${data.overall || 'Neutral'}</span><span class="text-3xl font-black font-headline">${score}%</span></div>
